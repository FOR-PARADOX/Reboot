...
namespace OpenBullet
{
    ...
    public partial class ConfigManager : Page
    {
        ...
        public void toSearcho(int op, string strings)
        {
            if (op == 2)
            {
                vm.SearchRefreshListX(strings);
            }
            else
            {
                vm.SearchRefreshListS(strings);
            }
        }
        ...
        private void loadConfigButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Current.Path.Contains(".loliX") && !Current.Config.Settings.isWtoEdit)
            {
                Globals.LogError(Components.ConfigManager, "The Author disabled Config Edits.", true);
                Globals.mainWindow.ConfigsPage.menuOptionStacker.IsEnabled = false;
                Globals.mainWindow.ConfigsPage.menuOptionOtherOptions.IsEnabled = false;
                flag = true;
                return;
            }           
            if (Current != null && !flag)
            {
                ...
                if (Current.Config.Settings.Author.Contains("(loliX Encrypted)") && !Current.Config.Settings.isWtoEdit)
                {
                    Globals.LogError(Components.ConfigManager, "The Author disabled Config Edits.", true);
                    Globals.mainWindow.ConfigsPage.menuOptionStacker.IsEnabled = false;
                    Globals.mainWindow.ConfigsPage.menuOptionOtherOptions.IsEnabled = false;
        ...
        public void SaveConfig()
        {
            ...
            if (Current.Path.Contains(".loliX"))
            {
                if (!IOManager.SaveConfigX(Current.Config, Current.Path))
                {
            ...
            else
            {
                ...
                if (!IOManager.SaveConfigX(Current.Config, Current.Path + "X"))
                {
                    Globals.LogError(Components.ConfigManager, "Failed to save the config to file.", true);
                    return;
                };
        ...
        {
            MessageBox.Show("Only Configs Disinfectors are allowed to use that feature!", "OpenBullet CyberSecurity");
        }
    }
}
