...
namespace RuriLib
{
    ...
    public class BlockFunction : BlockBase
    {
        ...
        public enum Function
        {
            ...
            /// <summary>Encrypts a string with AES (CBC MODE).</summary>
            AESEncryptCBC,

            /// <summary>Decrypts an AES-encrypted string (CBC MODE).</summary>
            AESDecryptCBC,

            /// <summary>Encrypts a string with AES (CFB MODE).</summary>
            AESEncryptCFB,

            /// <summary>Decrypts an AES-encrypted string (CFB MODE).</summary>
            AESDecryptCFB,

            /// <summary>Encrypts a string with AES (CTS MODE).</summary>
            AESEncryptCTS,

            /// <summary>Decrypts an AES-encrypted string (CTS MODE).</summary>
            AESDecryptCTS,

            /// <summary>Encrypts a string with AES (ECB MODE).</summary>
            AESEncryptECB,

            /// <summary>Decrypts an AES-encrypted string (ECB MODE).</summary>
            AESDecryptECB,

            /// <summary>Encrypts a string with AES (OFB MODE).</summary>
            AESEncryptOFB,

            /// <summary>Decrypts an AES-encrypted string (OFB MODE).</summary>
            AESDecryptOFB
        }
            ...
            for(int i = 0; i < localInputStrings.Count; i++)
            {
                ...
                switch (FunctionType)
                {
                    ...
                    case Function.AESEncrypt:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 1);
                        break;

                    case Function.AESDecrypt:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 1);
                        break;

                    case Function.AESEncryptCBC:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 2);
                        break;

                    case Function.AESDecryptCBC:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 2);
                        break;


                    case Function.AESEncryptCFB:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 3);
                        break;

                    case Function.AESDecryptCFB:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 3);
                        break;

                    case Function.AESEncryptCTS:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 4);
                        break;

                    case Function.AESDecryptCTS:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 4);
                        break;

                    case Function.AESEncryptECB:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 5);
                        break;

                    case Function.AESDecryptECB:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 5);
                        break;

                    case Function.AESEncryptOFB:
                        outputString = AESEncrypt(ReplaceValues(aesKey, data), localInputString, 6);
                        break;

                    case Function.AESDecryptOFB:
                        outputString = AESDecrypt(ReplaceValues(aesKey, data), localInputString, 6);
                        break;
        ...
        public static string AESEncrypt(string key, string data, int mode)
        {
            string encData = null;
            ...
            try
            {
                ...
                switch (mode)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        encData = EncryptStringToBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CBC);
                        break;
                    case 3:
                        encData = EncryptStringToBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CFB);
                        break;
                    case 4:
                        encData = EncryptStringToBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CTS);
                        break;
                    case 5:
                        encData = EncryptStringToBytes_AesMULTI(data, keys[0], keys[1], CipherMode.ECB);
                        break;
                    case 6:
                        encData = EncryptStringToBytes_AesMULTI(data, keys[0], keys[1], CipherMode.OFB);
                        break;
                }
        ...
        public static string AESDecrypt(string key, string data, int mode)
        {
            string decData = null;
            byte[][] keys = GetHashKeys(key);
            ...
            try
            {
                ...
                switch (mode)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        decData = DecryptStringFromBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CBC);
                        break;
                    case 3:
                        decData = DecryptStringFromBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CFB);
                        break;
                    case 4:
                        decData = DecryptStringFromBytes_AesMULTI(data, keys[0], keys[1], CipherMode.CTS);
                        break;
                    case 5:
                        decData = DecryptStringFromBytes_AesMULTI(data, keys[0], keys[1], CipherMode.ECB);
                        break;
                    case 6:
                        decData = DecryptStringFromBytes_AesMULTI(data, keys[0], keys[1], CipherMode.OFB);
                        break;
                }
        ...
        private static string EncryptStringToBytes_AesMULTI(string plainText, byte[] Key, byte[] IV, CipherMode mode)
        {
            if (plainText == null || plainText.Length <= 0)
                throw new ArgumentNullException("plainText");
            if (Key == null || Key.Length <= 0)
        ...
        private static string DecryptStringFromBytes_AesMULTI(string cipherTextString, byte[] Key, byte[] IV, CipherMode mode)
        {
